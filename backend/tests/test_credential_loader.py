...
def test_load_claude_code_credential_from_direct_env(monkeypatch):
    ...
    monkeypatch.setenv("CLAUDE_CODE_OAUTH_TOKEN", "  dummy-token-env  ")

    cred = load_claude_code_credential()

    assert cred is not None
    assert cred.access_token == "dummy-token-env"
...
def test_load_claude_code_credential_from_anthropic_auth_env(monkeypatch):
    _clear_claude_code_env(monkeypatch)
    monkeypatch.setenv("ANTHROPIC_AUTH_TOKEN", "dummy-token-anthropic-auth")

    cred = load_claude_code_credential()

    assert cred is not None
    assert cred.access_token == "dummy-token-anthropic-auth"
    assert cred.source == "claude-cli-env"
...
def test_load_claude_code_credential_from_file_descriptor(monkeypatch):
    ...
    try:
        os.write(write_fd, b"dummy-token-fd")
        os.close(write_fd)
    ...
    assert cred is not None
    assert cred.access_token == "dummy-token-fd"
    assert cred.refresh_token == ""
...
def test_load_claude_code_credential_from_override_path(tmp_path, monkeypatch):
    ...
    cred_path.write_text(
        json.dumps(
            {
                "claudeAiOauth": {
                    "accessToken": "dummy-token-test",
                    "refreshToken": "dummy-refresh-test",
    ...
    assert cred is not None
    assert cred.access_token == "dummy-token-test"
    assert cred.refresh_token == "dummy-refresh-test"
...
def test_load_claude_code_credential_falls_back_to_default_file_when_override_is_invalid(tmp_path, monkeypatch):
    ...
    default_path.write_text(
        json.dumps(
            {
                "claudeAiOauth": {
                    "accessToken": "dummy-token-default",
                    "refreshToken": "dummy-refresh-default",
                    "expiresAt": 4_102_444_800_000,
                }
            }
        )
    ...
    assert cred is not None
    assert cred.access_token == "dummy-token-default"
    assert cred.refresh_token == "dummy-refresh-default"
    assert cred.source == "claude-cli-file"
